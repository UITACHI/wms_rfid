...
GO
IF OBJECT_ID(N'[dbo].[FK_FunctionRoleModule]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[RoleModule] DROP CONSTRAINT [FK_FunctionRoleModule];
GO
...
-- Creating foreign key on [Function_FunctionID] in table 'RoleModule'
ALTER TABLE [dbo].[RoleModule]
ADD CONSTRAINT [FK_FunctionRoleModule]
    FOREIGN KEY ([Function_FunctionID])
    REFERENCES [dbo].[Function]
        ([FunctionID])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_FunctionRoleModule'
CREATE INDEX [IX_FK_FunctionRoleModule]
ON [dbo].[RoleModule]
    ([Function_FunctionID]);
GO
